...
void HikvisionCamera::decodeCallback(int nPort, char *pBuf, int nSize, FRAME_INFO *pFrameInfo)
{
    ...
    if (lFrameType == T_YV12)
    {
        ...
        ci->header.stamp = ros::Time::now(); // 头文件时间是当前时间，而非快门时间
...
bool HikvisionCamera::initHikSDK()
{
    ...
    user_id = NET_DVR_Login_V40(&struLoginInfo, &struDeviceInfoV40);// 登陆
...
void HikvisionCamera::initROSIO(ros::NodeHandle& priv_node)
{
    ...
    // CameraInfoManager 三个参数：node， camera_name(string), camera_info_url相机的配置文件所在的路径
